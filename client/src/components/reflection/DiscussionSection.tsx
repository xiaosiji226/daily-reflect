...
export default function DiscussionSection({ messages, onSend, loading, onReopen, reopening }: Props) {
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-5 animate-slide-up">
      <div className="flex items-center gap-2 mb-4">
        ...
        <div>
          <span className="text-sm font-semibold text-slate-700">回响</span>
          <span className="text-xs text-slate-400 ml-1.5">AI 对话伙伴</span>
        </div>
        {onReopen && (
          <button
            type="button"
            onClick={onReopen}
            disabled={reopening}
            className="ml-auto text-xs text-teal-500 font-medium active:text-teal-600 disabled:opacity-50 transition-colors"
          >
            {reopening ? '生成中...' : '重新生成'}
          </button>
        )}
